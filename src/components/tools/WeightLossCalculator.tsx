...
export default function WeightLossCalculator() {
  ...
  const [bodyFat, setBodyFat] = useState<string>(''); // 可選

  // 計算結果狀態
  const [result, setResult] = useState<{
    bmi: number;
    bmiStatus: 'underweight' | 'normal' | 'overweight' | 'mild_obesity' | 'severe_obesity';
    idealWeight: number; // 以 BMI 22 計算
    bmr: number; // 基礎代謝率
    bodyFatStatus?: 'low' | 'normal' | 'high' | 'very_high';
    bmrPosition: number; // 用於視覺化 BMI Bar 的位置 (0-100%)
  } | null>(null);
  ...
  const calculate = () => {
    ...
    else if (bmi >= 27 && bmi < 30) status = 'mild_obesity'; // 輕度肥胖
    else if (bmi >= 30) status = 'severe_obesity'; // 中重度肥胖
    ...
    // 3. 計算理想體重 (以 BMI 22 為標準)
    const idealW = 22 * heightInMeters * heightInMeters;

    // 4. 計算 BMR (Mifflin-St Jeor 公式)
    // 男：10 x 體重 + 6.25 x 身高 - 5 x 年齡 + 5
    // 女：10 x 體重 + 6.25 x 身高 - 5 x 年齡 - 161
    let bmrVal = 0;
    if (gender === 'male') {
      ...
      <div className="grid lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-5 space-y-8">
          ...
          <div className="bg-slate-700 p-2 rounded-2xl flex relative border border-slate-600">
            ...
            >
              <span className="text-2xl">👨</span> 男生
            </button>
            <button
            ...
            >
              <span className="text-2xl">👩</span> 女生
            </button>
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="group">
                <label className="block text-sm font-bold text-slate-300 mb-2">年齡 (歲)</label>
                <input
                  type="number"
                  value={age}
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="group">
                <label className="block text-sm font-bold text-slate-300 mb-2">目前體重 (kg)</label>
                <input
                  type="number"
                  value={weight}
